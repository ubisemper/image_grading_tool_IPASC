...
class S3StorageService(StorageService):
    def __init__(self, credentials):
        self.s3 = boto3.client(
            "s3",
            endpoint_url=credentials["end_point_url"],
    ...
    def get_file_names_in_folder(self, folder_name):
        file_names = self.get_all_file_names()
        filtered_file_names = [
            file for file in file_names if file.startswith(folder_name + "/")
        ]
        return filtered_file_names

    def get_file_by_name(self, file_name):
        try:
            file = self.s3.get_object(Bucket=self.bucket_name, Key=file_name)[
                "Body"
            ].read()
